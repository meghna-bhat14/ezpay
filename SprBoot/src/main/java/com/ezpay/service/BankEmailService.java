...
public class BankEmailService {
    ...
    public void sendEmail(String to, String subject, BankTransfer transactionDetails, String templateName, String senderName, String receiverName,Double balance) throws MessagingException {
        MimeMessage mimeMessage = mailSender.createMimeMessage();
        MimeMessageHelper helper = new MimeMessageHelper(mimeMessage, true);

        // Prepare the email context (Thymeleaf variables)
        Context context = new Context();
        context.setVariable("transactionId", transactionDetails.getTransferId());
    ...
    public void sendTransactionSuccessEmail(String senderEmail, String receiverEmail, BankTransfer transactionDetails) throws MessagingException {
        // Fetch sender and receiver details using UPI IDs
        Customer sender = customerRepo.findByBankAccountNumber(transactionDetails.getSenderAccountNumber()).orElse(null);
        Customer receiver = customerRepo.findByBankAccountNumber(transactionDetails.getReceiverAccountNumber()).orElse(null);
