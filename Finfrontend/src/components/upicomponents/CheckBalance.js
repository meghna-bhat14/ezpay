...
//     const senderUpiIdFromState = state?.senderUpiId; // Get senderUpiId from state
//     const navigate = useNavigate(); // Initialize navigate hook

//     const [upiId, setUpiId] = useState(senderUpiIdFromState || ''); // Initialize with state or empty
//     const [transactionPin, setTransactionPin] = useState(''); // State for transaction PIN
//     const [error, setError] = useState(''); // State to store error messages

//     // Function to handle fetching the balance
//     const getBalance = async () => {
//         try {
//             setError(''); // Reset any previous error messages

//             // Make a GET request to the backend API
//             const response = await fetch(`http://localhost:9020/api/get-balance?senderUpiId=${encodeURIComponent(upiId)}&SenderUpiPin=${encodeURIComponent(transactionPin)}`);
...
//             // Navigate to BalancePage with the balance and UPI ID passed as state
//             navigate('/balance', { state: { balance: balanceText, upiId: upiId } });
//         } catch (err) {
...
//     return (
//         <>
//             <Header title="Check Your Balance" />
//             <div style={{ textAlign: 'center', marginTop: '50px', height: "70vh" }}>
//                 <h2>Check Bank Account Balance</h2>
//                 {senderUpiIdFromState ? (
//                     <p>UPI ID: {senderUpiIdFromState}</p> // Display the fetched UPI ID
//                 ) : (
//                     <input
//                         type="text"
//                         value={upiId}
            ...
            <div style={{
                ...
                {senderUpiIdFromState ? (
                    ...
                    <input
                        ...
                        style={{
                            padding: '10px',
                            width: '470px', // Full width of the card
                            margin: '10px -10px',
